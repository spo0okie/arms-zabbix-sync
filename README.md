...
если таким образом узел не находится, то для ОС ищется соответствие по FQDN, для оборудования по IP
  * затем каждый узел Inventory обходит все наборы правил  
    ...
    * после обхода всех наборов правил над узлом Zabbix будут выполнены все накопленные действия из       
...
```
Пустой набор условий считается выполняющимся всегда (можно ставить в конце набора условий как действие по умолчанию)
