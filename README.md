...
### Отладка конвейера
Аргумент `verbose` добавляет по каждому узлу пояснение, почему с ним ничего не сделано:
```bash
php ./sync.php verbose
```

Аргумент `debug=узел1,узел2` включает подробный вывод работы конвейера **только по перечисленным узлам**
(без полного дампа по всем). Узел опознаётся без учета регистра по любому из полей: `fqdn`, `num`, `hostname` или `id`
(для ОС удобно указывать FQDN, для оборудования — инвентарный номер):
```bash
php ./sync.php debug=srv01.domain.local
php ./sync.php debug=srv01.domain.local,ARM-00123 verbose
php ./sync.php real debug=srv01.domain.local     # можно совмещать с боевым прогоном
```
Для каждого отлаживаемого узла по каждому набору правил печатается:
  * какое правило сработало (`MATCH`) — с его условиями и действиями;
  * какие правила пропущены (`skip`) и **на каком именно условии** отвалились (ключ к вопросу «почему тег/шаблон не назначился»);
  * если в наборе не совпало ни одно правило;
  * в конце — итоговый набор действий (`actions`) после подстановки макросов `${inventory:*}` в формате JSON.

### Explain-режим: «попадет ли узел в мониторинг» (для интеграции с ARMS)
Скрипт `explain.php` отвечает на вопрос «должен ли узел попадать в мониторинг и в каком виде»
по **одному** узлу инвентори. В отличие от sync.php он не подключается к Zabbix и не грузит
bulk-кэши инвентори (только один GET за узлом), поэтому отвечает за доли секунды и пригоден
для вызова из карточки узла ARMS (провайдер интеграции). Ничего никуда не пишет.

```bash
php ./explain.php comps 123      # CLI: класс узла (comps|techs) и его id в инвентори
```
HTTP-вариант (тот же скрипт за веб-сервером):
`GET /explain.php?class=comps&id=123&token=<токен>` — токен задается переменной
`$explainToken` в _config.priv.php_ (без настроенного токена HTTP-доступ закрыт);
принимается также заголовком `Authorization: Bearer <токен>`.

Ответ — JSON:
  * `verdict` — итог:
    * `add` — будет добавлен в мониторинг при следующей синхронизации;
    * `monitored` — уже привязан к Zabbix (по `external_links: Zabbix.hostid`) и будет обновляться;
    * `update-only` — не привязан, а создание правилами не разрешено — не добавится;
    * `declined` — правила явно запретили мониторинг (см. `errors`);
    * `skip` — конвейер не назначил действий;
  * `errors` — причины отказа (тексты из действий `errors` правил);
  * `status` — итоговый статус узла (0 — мониторится, 1 — приостановлен);
  * `sets` — трейс по каждому набору правил: какое правило совпало, какие пропущены и на каком условии;
    имена/описания наборов и правил — если заданы в rules.priv.php (см. ниже);
...
#### Имена и описания наборов/правил (опционально)
Наборам и правилам можно давать человекочитаемые имена — они появляются в отладочном
выводе (`debug=`) и в отчете `explain.php` вместо безликих `set#0 rule#3`:
  * **имя набора** — строковый ключ верхнего уровня массива правил;
  * **описание набора** — зарезервированный ключ `'desc'` внутри набора (строковые ключи внутри
    набора правилами не считаются);
  * **описание правила** — зарезервированный ключ `'desc'` внутри правила (элементы `[0]` и `[1]` —
    по-прежнему условия и действия).

```php
return [
	'Поддерживаемые ОС и оборудование'=>[            //имя набора
		'desc'=>'Что вообще умеем мониторить',       //описание набора (опционально)
		[
			['type'=>'comps','OS'=>['Windows']],
			['actions'=>['update','create'],'templates'=>['Windows by Zabbix agent']],
			'desc'=>'Windows серверы',               //описание правила (опционально)
		],
		[[],['errors'=>'эту ОС мониторить не умеем']],
	],
	[ /* безымянный набор в старом формате — работает как раньше */ ],
];
```
Все имена/описания **опциональны**: старый rules.priv.php без единого имени работает без
изменений, наборы и правила в выводе именуются номерами (`set#N`, `rule#M`).
...
```
  ...
  * serviceRecursive - то же что и service, но с учётом вложенности сервисов: совпадение засчитывается, если узел входит в сервис, который (возможно через цепочку родительских сервисов) входит в один из указанных. Точное написание имён.
```php
['serviceRecursive'=>['b'],...]; // совпадёт если узел в сервисе 'd', входящем в 'c', входящий в 'b'
['serviceRecursive'=>'*']; // только для узлов на которых крутится какой либо сервис
['serviceRecursive'=>false]; // только для узлов на которых нет сервисов
```
...
```
Внимание: для оборудования (ПК и т.п.), у которого свой IP адрес в инвентори не указан, но указана основная операционная система с адресом, IP адрес будет взят из неё.
...
```
  ...
  * teamNames - узел размещен сопровождается сотрудником (ФИО)
```php
['type'=>'comps','teamNames'=>['Пупкин Иван Петрович','Лоханкин Васиссуалий Андреевич']]; //компьютеры из песочницы bitrix clone
['type'=>'comps','teamNames'=>['*']]; //сопровождается кем угодно
['type'=>'comps','teamNames'=>[false]]; //никем не сопровождается
```

  * teamLogins - узел размещен сопровождается сотрудником (login)
```php
['type'=>'comps','teamLogins'=>['pupkin','lohankin']]; //компьютеры из песочницы bitrix clone
['type'=>'comps','teamLogins'=>['*']]; //сопровождается кем угодно
['type'=>'comps','teamLogins'=>[false]]; //никем не сопровождается
```
...
Техническое имя (host) Zabbix принимает только из ограниченного набора символов: латиница, цифры,
пробел, точка, дефис, подчёркивание. Кириллицу и скобки он отвергает. Конвейер проверяет это сам:
узел с недопустимым host получит понятную ошибку ещё на сухом прогоне, до обращения к API.
Чинится модификаторами макросов (см. ниже): `'host'=>'${inventory:num|translit}'`.
```php
[//сервер Zabbix
...
К любому макросу можно применить модификаторы: `${inventory:num|translit|normalize}`.
Значение прогоняется через них слева направо. Никакой скрытой магии - без модификатора
значение подставляется как есть (и если оно не годится в host, конвейер скажет об этом явно):

  * `|translit` - транслитерация кириллицы в латиницу (`МХК-NET-0079` -> `MHK-NET-0079`).
    Нужна для host: Zabbix не принимает кириллицу в техническом имени
  * `|normalize` - всё недопустимое для host заменяется на `_`, повторы `_` схлопываются,
    `_` и пробелы по краям обрезаются (`(SAP_2022)` -> `SAP_2022`). Удобно для суффиксов
    песочниц: разделитель приписывается в правиле явно
...
[// именование и адресация узлов
    ...
    [//оборудование именуем по инвентарному номеру, а обращаемся по IP
     //(в host кириллица не годится - транслитерируем; видимое имя остаётся кириллическим)
...
[// именование и адресация узлов
    [//компьютеры именуем по FQDN и обращаемся также
        ['type'=>'comps','sandbox'=>'*'],[
            'name'=>'${inventory:hostname} (${inventory:sandboxSuffix})',
            'host'=>'${inventory:hostname}_${inventory:sandboxSuffix|translit|normalize}',
            'address'=>'${inventory:ip}',
        ],
    ],
];
```
